...
구현된 클래스로 과정을 표현해보면 아래와 같습니다.<br>
```
        │   NetworkManager   │    ServerSession    │    PacketSession    │    Session         │    Socket       
────────┽────────────────────┾─────────────────────┾─────────────────────┾────────────────────┾────────────────────
Send    │   Send 시작        │    byte[] 변환 후   │                     │    비동기 전송     │    서버 전송 
        │                    │    Send             │                     │                    │
방향    │   ──────────────>  │    ────────────>    │    ───────────>     │    ────────────>   │    ─────────>
────────┽────────────────────┾─────────────────────┾─────────────────────┾────────────────────┾────────────────────
Recv    │   Update() 를      │    PacketQueue 에   │                     │    패킷 validate   │    비동기 수신   
        │   틱 마다 실행해   │    큐잉             │                     │                    │
        │   PacketQueue 에   │                     │                     │                    │
        │   쌓인 로직을 실행 │                     │                     │                    │
방향    │   ┌────────>   ─┐  │    <────────────    │    <───────────     │    <────────────   │    <─────────
...
2. 클라이언트와 서버 소켓의 연결이 완료되면, SeverSession 에서 CustomHandler 를 등록합니다.
...
3. 2. 과정에서 소켓 연결이 완료되었을 때, Update() 를 실행해 PacketQueue 에서 수신한 task 가 있는지 확인 후 있으면 실행합니다.
